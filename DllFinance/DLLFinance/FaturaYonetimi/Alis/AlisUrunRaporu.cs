...
{
    public partial class AlisUrunRaporu : UserControl
    {
        private DataTable alisFaturaTablosu;
        ...
        private void YukleVeri()
        {
            try
            {
                alisFaturaTablosu = SqlHelper.Instance.GetDataTable("CF_alisfaturaView");

                if (alisFaturaTablosu == null || alisFaturaTablosu.Rows.Count == 0)
                {
        ...
        {
            if (alisFaturaTablosu == null)
            {
                return null;
            }

            return alisFaturaTablosu.Columns.Cast<DataColumn>()
        ...
        private DataTable StokBazliOzetOlustur()
        {
            ...
            tablo.Columns.Add("AlisToplamMiktar", typeof(decimal));
            tablo.Columns.Add("AlisIadeToplamMiktar", typeof(decimal));
            tablo.Columns.Add("CiroToplami", typeof(decimal));

            var gruplanmis = alisFaturaTablosu.AsEnumerable()
            ...
            foreach (var grup in gruplanmis)
            {
                ...
                yeniSatir["AlisToplamMiktar"] = alisMiktari;
                yeniSatir["AlisIadeToplamMiktar"] = iadeMiktari;
        ...
        {
            if (string.IsNullOrEmpty(kolonAdi) || row == null || !alisFaturaTablosu.Columns.Contains(kolonAdi))
            {
                return 0;
            }
        ...
        private void GridAyarla()
        {
            ...
            KolonAyarla("AlisToplamMiktar", "Alış Toplam Miktarı", "n2", true);
            KolonAyarla("AlisIadeToplamMiktar", "Alış İade Toplam Miktarı", "n2", true);
        ...
        private void MiktarPastasiniCiz()
        {
            ...
            var data = stokOzetTablosu.AsEnumerable()
                .OrderByDescending(r => r.Field<decimal>("AlisToplamMiktar"))
                .Where(r => r.Field<decimal>("AlisToplamMiktar") > 0)
                .Take(20)
                .Select(r => new
                {
                    StokKodu = r.Field<string>("StokKodu"),
                    Miktar = r.Field<decimal>("AlisToplamMiktar")
                })
                .ToList();

            Series seri = new Series("Stok Miktarları", ViewType.Pie)
            {
                DataSource = data,
            ...
            chartMiktar.Titles.Add(new ChartTitle { Text = "Top 20 Stok - Alış Miktarı" });
        }
        ...
        private void CiroTop10Ciz()
        {
            ...
            chartCiro.Titles.Add(new ChartTitle { Text = "Top 10 Stok - Ciro (Birim Fiyat x Miktar)" });
        }
    }
}
